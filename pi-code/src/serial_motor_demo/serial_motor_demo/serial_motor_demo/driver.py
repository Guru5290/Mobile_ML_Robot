...
class MotorDriver(Node):
    ...
    def __init__(self):
        ...
        self.conn = serial.Serial(self.serial_port, self.baud_rate, timeout=3.0)
    ...
    def send_command(self, cmd_string):
     self.mutex.acquire()
     try:
        # Append newline to command string
        cmd_string += "\n"
        ...
        while time.time() - start_time < self.conn.timeout:
        ...
        if not response:
            print(f"Error: Serial timeout on command: {cmd_string.strip()}")
            return ''
