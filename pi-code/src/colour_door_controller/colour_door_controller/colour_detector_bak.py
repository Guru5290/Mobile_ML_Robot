...
class RTSPImagePublisher(Node):
    ...
    def detect_colour(self, imageFrame):
        ...
        white_upper = np.array([179, 50, 255], np.uint8)
        white_mask = cv2.inRange(hsvFrame, white_lower, white_upper) # actually white 
        ...
        counts = {"White": 0, "Blue": 0}
        max_areas = {"White": 0, "Blue": 0}
        ...
        def count_and_draw(mask, color_bgr, label):
            ...
            for contour in valid_contours:
                if cv2.contourArea(contour) > max_areas[label]:
                    max_areas[label] = cv2.contourArea(contour)
        ...
        count_and_draw(white_mask, (255, 255, 255), "White")
        ...
        if self.last_published_colour is None:
            ...
            max_key = max(max_areas, key=max_areas.get)
            max_value = max_areas[max_key] # max area
            self._publish_colour(max_key, counts)
